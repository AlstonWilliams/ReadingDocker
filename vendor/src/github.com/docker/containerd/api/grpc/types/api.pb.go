...
/* Reading:
 *  This method create a server-side stream.
 *  Let's think stream in grpc:
 *    Firstly, look at bidirectional stream in grpc. Client sends message to server in stream and server sends message to client simultaneously in stream.
 *    It is very easy to understand.
 *    And we can think client-side stream and server-side stream as a variant of bidirectional stream.
 *    Here we think server-side stream to illustrate, server-side stream is same as client just send one message then close the stream in bidirectional stream.
 *
 *  Ok, you have understand why we create a client stream but just send one message now.
 *
 *  You can study the code given by grpc which is located at <url>https://github.com/grpc/grpc-go/tree/master/examples/route_guide</url> to understand how stream works.
 *  You should also read the go source file generated by protoc compiler, and it is the most import thing to understand the principle.
 *
 */
